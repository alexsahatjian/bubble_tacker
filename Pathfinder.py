...
'''
def pathFinder(maxDiffValue,outputPath,X01,Y01,P2CM1,Tilt1,X02,Y02,P2CM2,Tilt2):
    maxDiff=maxDiffValue
    def findPathMatches(unmatched,toMatch,maxDiff):
        complicationsList=[]
        removeList=[]
    ...
    openFileA=outputPath+"BubblesA.txt"
    file=open(openFileA)
    data=file.read()
    data=data.split('|||\n')
    ...
    originX,originY=X01,Y01
    pixleToCM=P2CM1
    angleX=(Tilt1/360)*np.pi*2
    ...
    '''Read data from BubbleTracker.m'''
    openFileB=outputPath+"BubblesB.txt"
    file=open('openFileB)
    dataB=file.read()
    dataB=dataB.split('|||\n')
    ...
    BoriginX,BoriginY=X02,Y02
    BpixleToCM=P2CM2
    BangleX=(Tilt2/360)*np.pi*2
    ...
    writeFileA=outputPath+"A_paths.txt"
    writeFileB=outputPath+"B_paths.txt"
    writeFile=open(writeFileA,"w")
    writeFile2=open(writeFileB,"w")
